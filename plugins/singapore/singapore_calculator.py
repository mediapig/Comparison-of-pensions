...
class SingaporePensionCalculator(BasePensionCalculator):
    ...
    def calculate_pension(self,
        ...
        # 计算CPF账户余额
        cpf_balance = self._calculate_cpf_balance(
            contribution_history, economic_factors
        )

        # 计算月退休金（CPF Life年金）
        monthly_pension = self._calculate_cpf_life_pension(
            cpf_balance, retirement_age, economic_factors
        )
    ...
    def calculate_contribution_history(self,
        ...
        for year in range(work_years):
            ...
            # 新加坡CPF有缴费上限（2024年约为6,000新币）
            cpf_ceiling = 6000

            # 个人缴费（20%）
            personal_contribution = min(salary * 0.20, cpf_ceiling * 0.20)

            # 雇主缴费（17%）
            employer_contribution = min(salary * 0.17, cpf_ceiling * 0.17)
    ...
    def _calculate_cpf_balance(self,
        ...
        for i, record in enumerate(contribution_history):
            ...
            future_value = self.calculate_future_value(
                ...
                economic_factors.social_security_return_rate
            )
            total_balance += future_value

        return total_balance

    def _calculate_cpf_life_pension(self,
                                  cpf_balance: float,
                                  retirement_age: int,
                                  economic_factors: EconomicFactors) -> float:
        """计算CPF Life年金"""
        ...
        # 基础提取率
        base_withdrawal_rate = 0.045  # 4.5%
        ...
        # 计算年退休金
        annual_pension = cpf_balance * base_withdrawal_rate * age_factor
