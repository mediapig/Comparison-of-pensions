...
def analyze_australia_only(engine: PensionEngine, scenario_name: str, monthly_salary: float):
    """分析澳大利亚养老金"""
    desc = "" if "" else ""
    print_analysis_header("🇦🇺", "澳大利亚", scenario_name, monthly_salary, desc)

    # 创建标准对象
    person = create_standard_person()
    salary_profile = create_standard_salary_profile(monthly_salary, growth_rate=0.00)
    economic_factors = create_standard_economic_factors(base_currency="CNY", display_currency="AU")

    # 获取澳大利亚计算器
    calculator = engine.calculators["AU"]
